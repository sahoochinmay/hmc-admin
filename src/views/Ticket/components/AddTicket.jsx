...
import { addTicket } from "../../../redux/action/ticket.action";
...
const AddTicket = () => {
  ...
  const [speedoReading, setSpeedoReading] = useState();
  ...
  const handleTicketAdd = () => {
    // let currentWork = [...work]
    // currentWork.map((data,index) => {
    //   if (data?.description === "" && data?.quantity === ""  && data?.unitPrice === "" && data?.gstTotal === "") {
    //     currentWork.splice(index, 1);
    //   }
    // })
    // setWork([...currentWork]);
  ...
  function roundFloat(num, n) {
    return parseFloat(
      Number(Math.round(parseFloat(num + "e" + n)) + "e-" + n).toFixed(n)
    );
  }
